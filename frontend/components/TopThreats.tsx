    ...
    return (
      <div className="bg-white dark:bg-black p-4 rounded-lg border border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-lg font-semibold text-blue-600 dark:text-blue-400">Top 5 Critical Threats</h2>
        </div>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">No threats have been analyzed yet. Analyze a threat to see rankings here.</p>
    </div>
    )
  }
      ...
      <ol className="space-y-3">
        {topThreats.map(({ log, analysis }, index) => (
          <li
            ...
            className="flex items-center gap-3 p-2 rounded-md cursor-pointer transition-colors duration-200 bg-gray-100 dark:bg-gray-900 hover:bg-gray-200 dark:hover:bg-gray-800"
          >
            <span className="text-lg font-bold text-gray-400 dark:text-gray-600">{index + 1}</span>
            <div className="flex-grow">
              <p className="text-sm font-semibold text-black dark:text-white truncate">{analysis.threatName}</p>
              <p className="text-xs text-gray-600 dark:text-gray-400 font-mono">{log.sourceIp}</p>
            </div>
