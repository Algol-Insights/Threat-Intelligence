...
// ── Streamlined navigation — 7 core sections ────────────────────────────────
// Removed: Training, Attack Heatmap (merged into dashboard), Assets (in Settings)
// SOAR Playbooks, Threat Intel, MITRE, Reports, Executive Briefing → accessible via Settings or direct URL
...
const NAV = [
  ...
  { to: '/settings', icon: Settings, label: 'Settings', role: 'admin', key: '5' },
];
...
function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-30 flex flex-col bg-soc-surface border-r border-soc-border transition-all duration-300 ${collapsed ? 'w-16' : 'w-56'}`}>
        ...
        {NAV.filter(n => !n.role || n.role === user?.role).map(item => {
          ...
          return (
            <NavLink key={item.to} to={item.to}
              ...
              <item.icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'text-brand-400' : ''}`} />
              {!collapsed && (
                <>
                  <span className="truncate">{item.label}</span>
                  {item.accent && <span className="ml-auto text-[9px] bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded-full font-semibold">NEW</span>}
      ...
      {/* Keyboard hint */}
      {!collapsed && (
        <div className="px-4 py-2 border-t border-soc-border/50">
          <p className="text-[9px] text-soc-muted/50">Alt+1-5 to navigate</p>
        </div>
      )}
  ...
  return (
    <header className={`fixed top-0 right-0 z-20 h-12 border-b border-soc-border bg-soc-bg/80 backdrop-blur-sm flex items-center px-5 gap-4 transition-all ${collapsed ? 'left-16' : 'left-56'}`}>
      ...
      <div className="ml-auto flex items-center gap-3">
        <button onClick={toggleSound} className="text-soc-muted hover:text-soc-text transition-colors" title={soundEnabled ? 'Mute alerts' : 'Unmute alerts'}>
...
export default function App() {
  ...
  // Listen for critical events → toast + sound
  useEffect(() => {
    if (!isAuthenticated) return;
    const unsubs = [
      wsService.on('CORRELATION_ALERT', (alert: any) => {
        if (alert.severity === 'Critical' || alert.severity === 'High') {
          addToast(`${alert.ruleName} — ${alert.sourceIp} → :${alert.destinationPort}`, alert.severity);
          if (localStorage.getItem('alertSound') !== 'false') wsService.playAlertSound(alert.severity);
        }
      }),
      wsService.on('CDPA_BREACH', (data: any) => {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const routes = ['/dashboard', '/events', '/compliance', '/incidents', '/settings'];
