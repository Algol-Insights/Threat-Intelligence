...
// Secondary pages (accessible via URL)
...
const NAV = [
  ...
  { to: '/hunting', icon: Target, label: 'Threat Hunting', key: '6' },
  { to: '/deception', icon: Radar, label: 'Deception', key: '7' },
  { to: '/threats', icon: Bug, label: 'Threat Intel', key: '8' },
  { to: '/network', icon: Network, label: 'Network', key: '9' },
  { to: '/reports', icon: FileBarChart, label: 'Reports', key: '0' },
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex flex-col bg-soc-surface border-r border-soc-border transition-all duration-300 ${collapsed ? 'w-16' : 'w-56'}`}>
      ...
      {!collapsed && (
        <div className="px-4 py-1.5 border-t border-soc-border/30">
          <p className="text-[8px] text-soc-muted/40">Alt+1-9,0 to navigate</p>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      wsService.on('UEBA_ANOMALY', (data: any) => {
        addToast(`Behavioral anomaly: ${data.anomaly?.description || 'Unusual activity detected'}`, 'High');
        if (localStorage.getItem('alertSound') !== 'false') wsService.playAlertSound('High');
      }),
    ];
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const routes = ['/dashboard', '/events', '/compliance', '/incidents', '/playbooks', '/hunting', '/deception', '/threats', '/network', '/reports', '/settings'];
