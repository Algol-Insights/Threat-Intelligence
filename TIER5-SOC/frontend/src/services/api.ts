...
export const api = {
  ...
  // UEBA
  getUEBASummary: () => get<any>('/ueba/summary'),
  getUEBAAnomalies: (params?: Record<string, string>) => { const qs = params ? '?' + new URLSearchParams(params).toString() : ''; return get<{ anomalies: any[] }>(`/ueba/anomalies${qs}`); },

  // Honeypots / Deception
  getHoneypotStatus: () => get<any>('/honeypots/status'),
  startHoneypot: (port: number) => post<any>('/honeypots/start', { port }),
  stopHoneypot: (port: number) => post<any>('/honeypots/stop', { port }),
  getHoneypotEvents: () => get<{ events: any[]; total: number }>('/honeypots/events'),

  // Threat Hunting
  getHuntHypotheses: () => get<{ hypotheses: any[] }>('/hunt/hypotheses'),
  createHuntHypothesis: (data: any) => post<{ id: string }>('/hunt/hypotheses', data),
  updateHuntHypothesis: (id: string, data: any) => patch<{ success: boolean }>(`/hunt/hypotheses/${id}`, data),
  huntSearch: (params: any) => post<{ events: any[]; total: number }>('/hunt/search', params),
