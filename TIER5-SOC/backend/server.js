...
const { updateBaseline, detectAnomalies, persistAnomaly } = require('./uebaEngine');
const { startAllHoneypots, getHoneypotStatus } = require('./honeypotEngine');
const { uebaRouter, honeypotRouter, huntRouter } = require('./routes/tier5Routes');
...
// Tier 5 SOC capabilities
app.use('/api/v1/ueba', authenticate, apiLimiter, uebaRouter);
app.use('/api/v1/honeypots', authenticate, apiLimiter, honeypotRouter);
app.use('/api/v1/hunt', authenticate, apiLimiter, huntRouter);
...
syslogServer.on('message', (msg, rinfo) => {
  ...
  if (cdpaClassification.cdpaRelevant) {
    ...
    if (cdpaClassification.notificationRequired) {
      ...
      sendAlert('cdpa_breach', { log: enrichedLog, classification: cdpaClassification }).catch(() => {});
    }
  }

  // Step 5: PERSIST event to SQLite (REAL DATA)
  persistEvent(enrichedLog, cdpaClassification);

  // Step 5b: UEBA — update baseline and detect anomalies
  updateBaseline(logObject);
  const anomalies = detectAnomalies(logObject);
  for (const anomaly of anomalies) {
    persistAnomaly(logObject, anomaly);
    if (anomaly.score >= 0.7) {
      broadcast('UEBA_ANOMALY', { event: logObject, anomaly });
    }
  }

  // Step 6: Broadcast to all connected clients (ORIGINAL, preserved)
  broadcast('NEW_LOG', enrichedLog);

  // Step 7: Persist and broadcast correlation alerts (NEW)
  for (const alert of alerts) {
    persistAlert(alert);
    broadcast('CORRELATION_ALERT', alert);
    sendAlert('correlation_alert', alert).catch(() => {});
  }
});
...
const insertEvent = db.prepare(`
  INSERT INTO events (id, timestamp, source_ip, destination_ip, destination_port, protocol, action, description, parser_source, raw_log, misp_context, ioc_hit, cdpa_relevant, cdpa_classification, cdpa_risk_score, enrichment)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);

function persistEvent(log, cdpa) {
  try {
    insertEvent.run(
      log.id,
      log.timestamp,
      ...
      log.action,
      log.description,
      log.parserSource || null,
      log.rawLog || null,
      log.mispContext ? JSON.stringify(log.mispContext) : null,
      log.iocHit ? JSON.stringify(log.iocHit) : null,
      cdpa.cdpaRelevant ? 1 : 0,
      cdpa.classification || null,
      cdpa.riskScore || 0,
      null
    );
  } catch (err) {
    // Ignore duplicate ID errors on rapid ingestion
    if (!err.message.includes('UNIQUE')) console.error('[DB] Event persist error:', err.message);
  }
}
...
const insertAlert = db.prepare(`
  INSERT INTO alerts (id, rule_id, rule_name, severity, source_ip, destination_ip, destination_port, protocol, description, event_count, auto_responded, respond_action)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
function persistAlert(alert) {
  try {
    insertAlert.run(
      ...
      alert.description,
      alert.eventCount,
      alert.autoRespond ? 1 : 0,
      alert.respondAction || null
    );
  } catch (err) {
    if (!err.message.includes('UNIQUE')) console.error('[DB] Alert persist error:', err.message);
  }
}
...
server.listen(PORT, () => {
  ...
  // Start honeypot deception network
  try {
    const results = startAllHoneypots(broadcast);
    const started = results.filter(r => r.success).length;
    console.log(`  [HONEYPOTS] ${started} deception services started`);
  } catch (err) {
    console.log('  [HONEYPOTS] Deception services disabled:', err.message);
  }
});
