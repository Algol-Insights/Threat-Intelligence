...
import { Search, Zap, Loader2, Brain, X } from 'lucide-react';
...
export default function Events() {
  ...
  // Load REAL persisted events on mount
  useEffect(() => {
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-soc-surface">
              <tr className="text-soc-muted">
                ...
                <th className="text-left p-3 font-medium">Parser</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="p-8 text-center text-soc-muted">
                  <Loader2 className="w-5 h-5 animate-spin mx-auto mb-2" /> Loading events...
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="p-8 text-center text-soc-muted">
                  {logs.length === 0 ? 'No events recorded yet. Send syslog data to UDP :1514' : 'No events match filter'}
                </td></tr>
        ...
        <div className="w-full lg:w-[420px] shrink-0 flex flex-col bg-soc-surface rounded-xl border border-soc-border overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="space-y-2 text-xs">
              ...
              <Row label="Parser" value={selected.parserSource || 'unknown'} />
              {selected.cdpa?.cdpaRelevant && <Row label="CDPA" value={selected.cdpa.classification?.replace(/_/g, ' ') || 'relevant'} valueClass="text-emerald-400" />}
            </div>
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-white/[0.03] border border-soc-border">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={SEVERITY_BG[analysis.severity as string] || 'badge-info'}>{analysis.severity}</span>
                  </div>
                  <p className="text-xs text-soc-muted leading-relaxed">{analysis.summary}</p>
                </div>
                {analysis.cveId && analysis.cveId !== 'N/A' && <Section title="CVE" content={analysis.cveId} />}
