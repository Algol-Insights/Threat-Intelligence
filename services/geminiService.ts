  ...
  properties: {
    ...
    threatActorDNA: {
      ...
      properties: {
          ...
          ttps: {
              type: Type.STRING,
              description: "Key Tactics, Techniques, and Procedures (TTPs) associated with this actor and attack type, referencing MITRE ATT&CK where possible."
          },
          commonTools: {
              type: Type.STRING,
...
export const analyzeThreat = async (log: FirewallLog, context: OrganizationalContext | null): Promise<ThreatAnalysis> => {
  ...
  const prompt = `
    As a senior cybersecurity analyst, provide an expert-level, multi-faceted threat analysis of the following firewall log.
    ...
    **Analysis Dimensions:**
    ...
    3.  **Threat Actor DNA**: Profile the likely threat actor. Go beyond just a name; describe their typical TTPs (MITRE ATT&CK), tools, and motivations.
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('JSON')) {
        throw new Error("Gemini returned a malformed JSON. The model may be unable to generate a valid analysis for this log.");
    }
    throw new Error("Failed to parse threat analysis from Gemini.");
  }
};
...
export const getRemediationSuggestion = async (service: RunningService): Promise<string> => {
  ...
  try {
    ...
    throw new Error("Failed to generate remediation steps from Gemini.");
  }
};