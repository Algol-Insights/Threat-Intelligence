...
export default function Dashboard() {
  ...
  const highAlerts = correlationAlerts.filter(a => a.severity === 'High').length;
        ...
        <div className="card">
          ...
          <div className="h-48 flex items-center justify-center">
            {totalEvents === 0 ? <p className="text-soc-muted text-sm">Awaiting events...</p> : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart><Pie data={[{ name: 'Blocked', value: blockedCount, color: '#ef4444' }, { name: 'Allowed', value: allowedCount, color: '#22c55e' }]} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={4} dataKey="value">{[{ color: '#ef4444' }, { color: '#22c55e' }].map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ background: '#1a2236', border: '1px solid #1e293b', borderRadius: '8px', fontSize: '12px' }} /></PieChart>
              </ResponsiveContainer>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card">
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {correlationAlerts.length === 0 ? <p className="text-soc-muted text-sm text-center py-8">No alerts yet — monitoring...</p> :
            correlationAlerts.slice(0, 10).map(alert => (
              <div key={alert.id} className={`card-compact flex items-start gap-3 ${alert.severity === 'Critical' ? 'animate-threat-pulse border-red-500/30' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {alert.autoRespond && <span className="text-[10px] text-brand-400 font-medium">Auto-blocked</span>}
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="card">
          ...
          <div className="space-y-1 max-h-80 overflow-y-auto font-mono text-xs">
            ...
            logs.slice(0, 20).map(log => (
              <div key={log.id} className="flex items-center gap-2 py-1.5 px-2 rounded hover:bg-white/5 transition-colors cursor-pointer" onClick={() => nav('/events')}>
